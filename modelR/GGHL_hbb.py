...
class GGHL(nn.Module): #Assigning Better Gaussian Heatmaps for Arbitrary Oritened Object Detection
    """
    ...
    def __init__(self, init_weights=True, inputsize= int(cfg.TRAIN["TRAIN_IMG_SIZE"])):
        ...
        self.__out_channel = self.__nC + 4 + 1   ###########class+xywh,a1-a4,r

        self.__backnone = Darknet53()#Darknet53_NPAttention()#Darknet53()
        self.__fpn = Neck(fileters_in=[1024, 512, 256], fileters_out=self.__out_channel)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            p, p_d = list(zip(*out))
            return p, torch.cat(p_d, 0)#p_d#torch.cat(p_d, 0)
