...
from modelR.necks.neck_NPMMR import MSR_FPN
from modelR.head.head_NPMMR import MTR_Head2
from modelR.layers.convolutions import Convolutional
from utils.utils_basic import *

class ABGH_NPMMRDet(nn.Module):
    """
    Note ： int the __init__(), to define the modules should be in order, because of the weight file is order
    """
    def __init__(self, init_weights=True):
        super(ABGH_NPMMRDet, self).__init__()
        ...
        # small
        self.__head_s = MTR_Head2(nC=self.__nC, stride=self.__strides[0])
        # medium
        self.__head_m = MTR_Head2(nC=self.__nC, stride=self.__strides[1])
        # large
        self.__head_l = MTR_Head2(nC=self.__nC, stride=self.__strides[2])
