...
class Head1(nn.Module):  # 先预测第一个框和偏移量
    ...
    def forward(self, input1):
        ...
        out1_de, offsets_loc, offset_cls = self.__decode(out1, offset_vertex)
        return out1, out1_de, offsets_loc, offset_cls, mask
    ...
    def __decode(self, p, offset_vertex):
        ...
        dc_off = torch.exp(offset_vertex)
        dc_off_y0 = off_y0 + dc_off[:, :, :, 0:1]
        dc_off_y2 = off_y2 + dc_off[:, :, :, 1:2]
        dc_off_y6 = off_y6 - dc_off[:, :, :, 2:3]
        dc_off_y8 = off_y8 - dc_off[:, :, :, 3:4]

        dc_off_x0 = off_x0 + dc_off[:, :, :, 4:5]
        dc_off_x2 = off_x2 - dc_off[:, :, :, 5:6]
        dc_off_x6 = off_x6 + dc_off[:, :, :, 6:7]
        dc_off_x8 = off_x8 - dc_off[:, :, :, 7:8]
        ...
        offset_cls = torch.cat([dc_off_y0, dc_off_x0, off_y1, off_x1, dc_off_y2, dc_off_x2,
                             off_y3, off_x3, off_y4, off_x4, off_y5, off_x5,
                             dc_off_y6, dc_off_x6, off_y7, off_x7, dc_off_y8, dc_off_x8]
                            , dim=-1).permute(0, 3, 1, 2).contiguous()

        return pred_bbox, offsets_loc, offset_cls
...
class Head2(nn.Module):
    ...
    def forward(self, out1_de, loc, cls, offsets_loc, offsets_cls, mask):

        loc_dcn = self.__dcn_loc(loc, offsets_loc, mask)
        ...
        cls_dcn = self.__dcn_cls(cls, offsets_cls, mask)
