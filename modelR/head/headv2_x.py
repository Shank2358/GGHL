...
from dcn_v2_amp import DCNv2, DCNv2_diff
...
class Head1(nn.Module):  # 先预测第一个框和偏移量
    def __init__(self, filters_in, stride):
        ...
        self.__conv = Convolutional(filters_in=filters_in + 2, filters_out=filters_in * 2, kernel_size=3, stride=1, pad=1, norm="bn", activate="leaky")
        self.weight = nn.Parameter(torch.Tensor(filters_in * 2, filters_in, 3, 3))
        self.weights_c = self.weight.sum(dim=[2, 3], keepdim=True)
        self.bias = nn.Parameter(torch.Tensor(filters_in * 2))
        ''''''
    ...
    def forward(self, input1):
        ...
        batch_size, output_size = input1.shape[:2]
        ...
        grid_xy = grid_xy.unsqueeze(0).repeat(batch_size, 1, 1, 1).float().to(input1.device)
        position_token = grid_xy.permute(0, 3, 1, 2).contiguous()
        input1 = torch.cat([input1, position_token], dim=1)

        conv = self.__conv(input1)
        #out1 = self.__loc1(conv).permute(0, 2, 3, 1).contiguous()
        out1c = F.conv2d(input1, self.weights_c, stride=1, padding=0, groups=1)
        out1ori = F.conv2d(input1, self.weights, self.bias, stride=1, padding=1, dilation=1, groups=1)
        out1 = (out1ori - out1c).permute(0, 2, 3, 1).contiguous()
...
class Head2(nn.Module):
    def __init__(self, filters_in, nC, stride):
        ...
        self.__dcn_loc_center = DCNv2_diff(in_channels=filters_in, out_channels=filters_in * 2, kernel_size=3, padding=0, stride=1)
    ...
    def forward(self, out1_de, loc, cls, offsets, mask):
        ...
        loc_dcn_center = self.__dcn_loc_center(loc, offsets, mask)
        loc_dcn = loc_dcn - loc_dcn_center
