...
class Construct_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return img, label_xsbbox, label_sbbox, label_mbbox, label_lbbox
    ...
    def __creat_label(self, label_lists=[]):
        self.gt_tensor = [np.zeros((int(self.img_size // self.stride[i]), int(self.img_size // self.stride[i]),
                               self.num_classes + 16)) for i in range(4)]
        layer_thresh = [self.img_size/10, self.img_size/5, self.img_size/2.5]
        for gt_label in label_lists:
            ...
            if max(box_w, box_h) > 10 or (box_w*box_w) > 80:
                ...
                if length > layer_thresh[1] and length <= layer_thresh[2]:
                    self.generate_label(2, self.gt_tensor, c_x_r, c_y_r, len_w, len_h, box_w, box_h, angle,
                                        ymin, xmax, ymax, xmin, c_x, c_y, a1, a2, a3, a4,
                                        gt_label, class_id)

                if length > layer_thresh[2]:
                    self.generate_label(3, self.gt_tensor, c_x_r, c_y_r, len_w, len_h, box_w, box_h, angle,
                                        ymin, xmax, ymax, xmin, c_x, c_y, a1, a2, a3, a4,
                                        gt_label, class_id)

        label_xsbbox, label_sbbox, label_mbbox, label_lbbox = self.gt_tensor
        return label_xsbbox, label_sbbox, label_mbbox, label_lbbox
...
if __name__ == '__main__':
    ...
    for i, (imgs, label_xsbbox, label_sbbox, label_mbbox, label_lbbox) in enumerate(train_dataloader):
        continue
