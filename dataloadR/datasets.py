...
class Construct_Dataset(Dataset):
    ...
    def __creat_label(self, label_lists=[]):
        self.gt_tensor = [np.zeros((int(self.img_size // self.stride[i]), int(self.img_size // self.stride[i]), self.num_classes + 16)) for i in range(3)]
        ratio = (1 - self.IOU_thresh)
        self.layer_thresh = [3*(self.stride[0]*2)/ratio, 3*(self.stride[2]*2)/ratio]
...
if __name__ == '__main__':
    ...
    train_dataset = Construct_Dataset(anno_file_name="small_angle", img_size=800)#cfg.TRAIN["TRAIN_IMG_SIZE"]
